...
def browse_dest_path():
  ...
  if folder_selected == "": # 사용자가 취소를 누를 때 
    # print("폴더 선택 취소") # 폴더 선택 취소 잘 되는지 확인 
    return
...
def merge_image():
  ...
  try:
    ...
    if img_width == "원본유지":
      ...
      img_width = int(img_width) # 정수형으로 저장 필요
    ...
    # 계산식 
    # 100 * 60 이미지가 있다면 -> width 를 80으로 줄이면 height 는 ?
    ...
    for idx, img in enumerate(images):
      # width 가 원본유지가 아닐 때에는 이미지 크기 조절 필요
      if img_width > -1:
        img = img.resize(image_sizes[idx])
        
      result_img.paste(img, (0, y_offset))
      y_offset += (img.size[1] + img_space) # height 값 + 사용자가 지정한 간격 더하기
    ...
    # 포맷 옵션 처리
    file_name = "photo_project." + img_format
